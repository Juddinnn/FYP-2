...
TTL_FEATURES = ["ttl_min", "ttl_max", "ttl_avg", "ttl_stddev"]

IP_DIVERSITY_FEATURES = [
    "num_A_records",
    "ip_entropy",
    "num_unique_subnets"
]

DNS_STRUCTURE_FEATURES = [
    "num_CNAME_records",
    "num_NS_records"
]
...
# ============================================================
# ABLATION SETTINGS (ONLY-FEATURE STUDY)
# ============================================================

ABLATION_CONFIGS = {
    "Only TTL Features": TTL_FEATURES,

    "Only IP Diversity Features": IP_DIVERSITY_FEATURES,

    "Only DNS Structure Features": DNS_STRUCTURE_FEATURES,
...
def run_rnn_experiment(X_sub, y, experiment_name):
    print(f"\n==============================")
    print(f"Running: {experiment_name}")
    print(f"Features used: {list(X_sub.columns)}")
    print(f"==============================")
    ...
    # ---------- MODEL ----------
    model = LSTM(X_sub.shape[1], 64).to(device)
    model = train_rnn(model, train_loader, val_loader)
...
# RNN TRAINING
# ============================================================
def train_rnn(model, train_loader, val_loader, epochs=50):
    model.to(device)
...
# ============================================================
# RUN RNN ABLATION STUDY
# ============================================================

ablation_results = {}

for exp_name, feature_list in ABLATION_CONFIGS.items():
    X_sub = X[feature_list]
    y_true, y_pred = run_rnn_experiment(X_sub, y, exp_name)
    ablation_results[exp_name] = (y_true, y_pred)
...
fig, axes = plt.subplots(1, len(ablation_results), figsize=(18, 4))

for ax, (name, (y_t, y_p)) in zip(axes, ablation_results.items()):
